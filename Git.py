...
for (start, end), _ in distances.items():
...
def dfs(current_node, path, all_paths):
    ...
    for next_node in connections[current_node]:
        if next_node not in path:
            dfs(next_node, path + [next_node], all_paths)

all_paths = []
dfs(start_node, [start_node], all_paths)
...
fuzzy_transport_time = {
    ...
    ("10", "11"): {"公路": (15, 16, 17), "铁路": (14, 16, 20), "水路": (0, 0, 0)},
    ("10", "D"): {"公路": (9, 10, 11), "铁路": (6, 8, 9), "水路": (0, 0, 0)},
...
def get_time_window(node):
    if node in ["1", "2", "3", "4", "5"]:
        return time_window["1-5"]["范围"]
    elif node in ["6", "7", "8", "9", "10", "11"]:
        return time_window["6-11"]["范围"]
    elif node == "D":
        return time_window["D"]["范围"]
    else:
...
"""def total_transport_cost(X, Y, distances, unit_fees, transfer_info, Pe, Pl, time_window):
    ...
    # 提前到达和延迟到达的成本
    early_cost = sum(
        Pe * max(get_fuzzy_time_window(node)[0] - get_fuzzy_ti_for_node(node, all_paths[0], fuzzy_transport_time, transfer_info)[1], 0)
        for node in middle_nodes
    )

    late_cost = sum(
        Pl * max(get_fuzzy_ti_for_node(node, all_paths[0], fuzzy_transport_time, transfer_info)[1] - get_fuzzy_time_window(node)[1], 0)
        for node in middle_nodes
    )

    return inter_node_cost + transfer_cost + early_cost + late_cost """
...
def calculate_fuzzy_ti_for_each_node(path, fuzzy_transport_time, transfer_info):
    """
    计算每个节点的模糊时间变量ti
    """
    # 初始化模糊时间为(0, 0, 0)
...
def get_fuzzy_ti_for_node(node, path, fuzzy_transport_time, transfer_info):
    ti_values = calculate_fuzzy_ti_for_each_node(path, fuzzy_transport_time, transfer_info)
...
X = pulp.LpVariable.dicts("X", ((i, j, k) for i in nodes for j in nodes for k in transport_modes if (i, j) in distances and k in distances[(i, j)] if i != j ), 0, 1, pulp.LpBinary)
Y = pulp.LpVariable.dicts("Y", ((i, k, l) for i in middle_nodes for k in transport_modes for l in transport_modes if k != l), 0, 1, pulp.LpBinary)

# 确定Y包含的所有可能的键组合
Y = {(i, k, l): 0 for i in middle_nodes for k in transport_modes for l in transport_modes if k != l}

# 定义目标函数
#omega=0.5  # 权重，可以根据需要调整
#model += omega * total_transport_cost(X, Y, distances, unit_fees, transfer_info, Pe, Pl, time_window) + (1 - omega) * total_carbon_emission(X, Y, distances, unit_emissions, transfer_info), "Objective"
...
# 定义决策变量,f_minus看作是一个决策变量
f_minus = pulp.LpVariable("f_minus", 0, None, pulp.LpContinuous)  # 设置f_minus的范围为[0, 无穷大)

# ... 其他约束 ...
# 约束11. f-的定义

    # 修改objective_function，返回一个pulp的LpAffineExpression对象，而不是一个布尔值
def objective_function(X, Y):
    term1 = pulp.lpSum([sum([sum([get_unit_fee(k) * get_distance(i, j, k) * X[i, j, k]
                           for k in transport_modes if (i, j, k) in X.keys()])
                      for j in nodes if j != start_node and (i, j) in distances.keys()])
                 for i in nodes if i != end_node])

    term2 = pulp.lpSum([sum([get_transfer_fee(i, k, l) * Y[i, k, l]
                      for l in transport_modes
                      for k in transport_modes
                      if k != l and (i, k, l) in Y.keys()])
                 for i in middle_nodes])

    term3 = pulp.lpSum([sum([sum([get_unit_emission(k) * get_distance(i, j, k) * X[i, j, k]
                           for k in transport_modes if (i, j, k) in X.keys()])
                      for j in nodes if j != start_node and (i, j) in distances.keys()])
                 for i in nodes if i != end_node])

    term4 = pulp.lpSum([sum([get_transfer_emission(i, k, l) * Y[i, k, l]
                      for l in transport_modes
                      for k in transport_modes
                      if k != l and (i, k, l) in Y.keys()])
                 for i in middle_nodes])
    ...
    term5 = sum([max(get_fuzzy_time_window(i)[0] - get_fuzzy_ti_for_node(i, path, fuzzy_transport_time, transfer_info)[0], 0) for i in nodes])
    term6 = sum([max(get_fuzzy_ti_for_node(i, path, fuzzy_transport_time, transfer_info)[1] - get_fuzzy_time_window(i)[1], 0) for i in nodes])


    return omega * ((1 - alpha) * q_l + alpha * q_m) * (term1 + term2 + Pe * term5 * (1 - alpha) + Pl * term6 * alpha)  + (1 - omega) * ((1 - alpha) * q_l + alpha * q_m) * (term3 + term4)

# 在模型中定义目标函数
model += objective_function(X, Y), "Objective"

# 约束11. f-的定义
model += f_minus >= objective_function(X, Y), "Objective_Constraint"

# 约束12. f-的上界
model += f_minus <= omega * max_cost + (1 - omega) * max_emission, "Upper_Bound_Constraint"
...
"""constraints.append(objective_function(X, Y))"""
...
print(f"Optimal value of f-: {f_minus.value}")

# 输出结果
print("Objective Value:", pulp.value(model.objective))
for var in X:
    if pulp.value(X[var]) > 0:
        print(var, pulp.value(X[var]))
for var in Y:
    if pulp.value(Y[var]) > 0:
        print(var, pulp.value(Y[var]))
